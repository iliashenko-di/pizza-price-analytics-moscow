...
- открытие конфигуратора каждой пиццы;
...
Есть 3 позиции, где карточка найдена, но вариации не снялись из конфигуратора:

- `Пицца Энчантикс`;
- `Додо`;
- `Диабло`.

Их нужно доснимать отдельным retry-проходом. В дашборде эти позиции отображаются отдельно, чтобы не смешивать неполный сбор с полноценными вариациями.
...
- Dodo-позиции, которые нужно доснять.
...
1. Добавить retry-проход для Dodo-позиций без вариаций.
2. Вынести ручные соответствия в отдельный конфигурационный файл.
3. Добавить GitHub Action для обновления `data/pizza-snapshot.json` по расписанию или вручную.
4. Добавить экспорт CSV/XLSX для аналитика.
